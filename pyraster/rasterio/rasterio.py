...
def newgdalraster(outfile, format, XSize, YSize, geotrans, epsg, num_bands,
                  gdal_dtype):
...
def newrasterband(dst_ds, rasterarray, band_num, NoDataVal=None):
    '''Accepts a GDAL dataset pointer, rasterarray, band number, [NoDataValue],
...
def writerasterbands(outfile, format, XSize, YSize, geotrans, epsg,
    ...
    #create new raster
    dst_ds = newgdalraster(outfile, format, XSize, YSize, geotrans, epsg,
                           num_bands, npy2gdt[rasterarrays[0].dtype.name])
...
#legacy function to write GeoTiff raster from NumPy n-dimensional array -
# use writerasterbands instead
def writerasterband(rasterarray, outfile, format, aXSize, aYSize, geotrans,
                    epsg, NoDataVal=None):
    ...
    geotranslation metadata and writes to file on disk.'''
    writerasterbands(outfile, format, aXSize, aYSize, geotrans, epsg,
                     NoDataVal, rasterarray)
