...
####1. PyRaster module (RasterIO Class)
  ...
  The RasterIO class uses the Geospatial Data Abstraction Library (GDAL)<sup>[1](#note-1)</sup> to read and write images and their geospatial metadata to and from NumPy arrays.
...
* Holderness, T. (2013) Quantifying the spatio-temporal temperature dynamics of Greater London using thermal Earth observation. <sup>[2](#note-2)</sup>
* PyRaster presentation to Mars Space Flight Facility in 2010. <sup>[3](#note-3)</sup>
* Raster Processing Suite QGIS plugin. <sup>[4](#note-4)</sup>

###Documentation
* RasterIO API documentation in HTML format can be found in the `doc` folder and is hosted at http://talltom.github.io/PyRaster/
* Instructions on using the software are provided below in this `README.md` file

###Supported Formats
* Input: Any GDAL supported format <sup>[5](#note-5)</sup>
...
####1.3 Reading a file
To read a raster file from disk and covert it to a NumPy array two RasterIO functions are required.
...
band_number = 1
b1 = rio.read_band(dataset, band_number)
...
Figure 1. Loading Raster Data into a NumPy array using the PyRaster RasterIO module (Adapted from Holderness, T. 2013 <sup>[2](#note-2)</sup>).

####1.4 Metadata Handling
The `read_metadata()` method complements `read_band()`,  to read the geospatial raster meta-data from a raster file. The `read_metadata()` function accepts a GDAL raster dataset (i.e. from `open`) and returns a dictionary containing the GDAL driver, number of rows, number of columns, number of bands, projection info (well known text), and geotranslation metadata.
...
PyRaster uses NumPy masked arrays <sup>[6](#note-6)</sup> to handle no data values in raster images. When masking is in place, masked values are not included in array calculations.

No Data Values (`NoDataValue`) are derived from individual band metadata using the `band.GetNoDataValue()` GDAL function <sup>[7](#note-7)</sup>.
...
###2 Raster Processing
NumPy Elementwise Array Operations <sup>[8](#note-8)</sup>
```python
...
```
NumPy Universal Functions <sup>[9](#note-9)</sup>
```python
print band.mean(), band.min(), band.max  #compute mean, min and max values from all cells in band
```

###3 Template Scripts
Complete versions of the scripts described below can be found in the `templates` folder.
...
NDVI is defined as a ratio between two bands, where band 1 is in the red visible wavelength and band 2 is near-infrared. <sup>[10](#note-10)</sup>
...
dataset = rio.open('file.tif')
#read the bands
b1 = rio.read_band(dataset, 1)
b2 = rio.read_band(dataset, 2)
ndvi = (b2-b1)/(b2+b1)
```

To run the script, do:
```bash
[pyraster]$ python scripts/batch.py
```

batch.py generates a new raster representing the NDVI values calculated. The suffix "-ndvi" is added to the output filename to distinguish it from the original file. The script is a simple example of iterating raster computation over a series of data.

####3.2 vectorize.py
Vectorized functions can improve NumPy performance. <sup>[11](#note-11)</sup> This script uses the vectorize process to create a Boolean output raster based on a threshold function.
...
####3.4 historgram.py
This script demonstrates creation of a band histogram plotted using the Matplotlib library. <sup>[13](#note-13)</sup>
...
####3.5 plot.py
The Matplotlib library can also be used to plot arrays as images using the `imshow` method. <sup>[14](#note-14)</sup> Figure 2 shows an example Landsat image plot.
...
```
Coverage is run by Travis-CI on commit, and Coveralls.io is used to report on coverage of tests.
###Release
...
```
Generated documentation should be copied over to gh-pages branch for hosting at http://talltom.github.io/PyRaster/

##Notes
<a name="note-1"></a>[1] Geospatial Data Abstraction Library http://gdal.org  
<a name="note-2"></a>[2] Holderness, T. 2013 PhD Thesis http://hdl.handle.net/10443/1856  
<a name="note-3"></a>[3] Holderness, T. 2010 PyRaster Presentation https://tomholderness.files.wordpress.com/2012/12/holderness_asu_pyraster_pres_handout.pdf  
<a name="note-4"></a>[4] Raster Processing Suite QGIS Plugin http://talltom.github.io/Raster-Processing-Suite/  
<a name="note-5"></a>[5] GDAL Format List http://www.gdal.org/formats_list.html  
<a name="note-6"></a>[6] NumPy Arrays
http://docs.scipy.org/doc/numpy/reference/maskedarray.html  
<a name="note-7"></a>[7] GDAL No Data Values http://www.gdal.org/classGDALRasterBand.html#adcca51d230b5ac848c43f1896293fb50  
<a name="note-8"></a>[8] NumPy Elementwise Array Operations http://wiki.scipy.org/Tentative_NumPy_Tutorial#head-4c1d53fe504adc97baf27b65513b4b97586a4fc5  
<a name="note-9"></a>[9] NumPy Universal Functions http://wiki.scipy.org/Tentative_NumPy_Tutorial#head-053463ac1c1df8d47f8723f470b62c4bd0d11f07  
<a name="note-10"></a>[10] Normalised Difference Vegetation Index https://en.wikipedia.org/wiki/Normalized_Difference_Vegetation_Index  
<a name="note-11"></a>[11] NumPy vectorize  http://docs.scipy.org/doc/numpy/reference/generated/numpy.vectorize.html  
<a name="note-12"></a>[12] Python Multiprocessing module https://docs.python.org/2/library/multiprocessing.html  
<a name="note-13"></a>[13] Plotting an image histogram with Matplotlib http://matplotlib.org/users/image_tutorial.html#examining-a-specific-data-range  
<a name="note-14"></a>[14] Plotting an image with Matploblib http://matplotlib.org/users/image_tutorial.html#image-tutorial
